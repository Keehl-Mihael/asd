...
template <class I>
void TreeList<I>::depth_search (node nodo,int &max_level) const{
	node nodo2;

    if (!leaf(nodo)) {
        nodo2 = firstChild(nodo);
        while (!lastSibling(nodo2)) {
            depth_search(nodo2, max_level);
            nodo2 = nextSibling(nodo2);
        }
        max_level--;
        depth_search(nodo2, max_level);
    }
    max_level++;
    cout << readNode(nodo2) << endl;

}


template <class I>
int TreeList<I>::depth (){
    int level = 0;
   depth_search(this->root(),level);
   return level;
}
